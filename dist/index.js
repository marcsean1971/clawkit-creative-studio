...
function makeProductAudit(params) {
    ...
    const weakFromCaptures = captures
        .filter((capture) => ["weak", "broken"].includes(capture.quality ?? "usable") ||
    ...
    const baseScore = marketability?.score ??
        clampScore(40 +
    ...
    const overallReadiness = score >= 85
        ? "ready-to-promote"
        : score >= 70
            ? "promising"
            : score >= 45
                ? "needs-work"
                : "blocked";
    const strongestScreens = approvedScreens.length > 0
    ...
    const recommendedPromoAngle = intelligence.launchAngles[0] ??
    ...
    return {
        ...
        lovableFixPrompts: params.isLovableApp
            ...
            : [],
        nextWorkflow: overallReadiness === "ready-to-promote"
            ? [
...
function makePromptExport(params) {
    ...
    const markdown = [
        ...
        ...(videoScenes.length > 0
            ? videoScenes.map((scene, index) => `${index + 1}. ${String(scene.time ?? "")} ${String(scene.title ?? "Scene")}: ${String(scene.direction ?? "")}`)
...
function makeAgencyReport(params) {
    ...
    const productSummary = params.productAudit?.productUnderstanding.summary ??
        params.intelligence?.productSummary ??
        "Product summary not supplied. Run `creative_site_intelligence` before final delivery.";
    const readiness = params.productAudit?.overallReadiness ??
        params.marketabilityAudit?.readiness ??
        "not audited";
    const score = params.productAudit?.score ??
        params.marketabilityAudit?.score ??
        "not scored";
    const strongestScreens = params.productAudit?.screenDiagnosis.strongestScreens ??
        params.marketabilityAudit?.strongestScreens ??
        params.captureReport?.approvedCaptures.map((capture) => capture.ref) ??
        [];
    const weakScreens = params.productAudit?.screenDiagnosis.weakOrBrokenScreens ??
        params.marketabilityAudit?.weakScreens ??
        params.captureReport?.weakCaptures.map((capture) => `${capture.ref} (${capture.url})`) ??
        [];
    const trustGaps = params.productAudit?.marketingDiagnosis.missingTrustSignals ??
        params.intelligence?.proofGaps ??
        params.marketabilityAudit?.missingEvidence ??
        [];
    const fixes = params.productAudit?.fixBeforePromotion ??
        params.marketabilityAudit?.recommendedFixes ??
        ["Complete Creative Audit Mode before sending a final client report."];
    const recommendedAssets = params.productAudit?.recommendedAssets.map((asset) => `- ${asset.asset}: ${asset.readiness}. Source screens: ${asset.sourceScreens.join(", ") || "not selected"}. ${asset.purpose}`) ??
        params.assetMatrix?.matrix?.map((item) => `- ${String(item.asset ?? "Asset")}: ${String(item.productionStatus ?? "status unknown")}. Source: ${String(item.screenshotRef ?? "not selected")}`) ??
    ...
    const scanSummary = typeof params.scanSummary === "string"
        ? params.scanSummary
        : params.scanSummary
            ? [
    ...
    const lovablePrompts = params.includeLovablePrompts
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        api.registerTool({
            ...
            description: "Inspect a scanned app like a product marketer, designer, and QA reviewer before generating promotional images, video storyboards, or launch copy.",
            parameters: Type.Object({
                intelligence: Type.Any(),
        ...
        api.registerTool({
            ...
            description: "Decide which markdown, JSON, and CSV handoff files should be created for a launch pack.",
            parameters: Type.Object({
                productName: Type.Optional(Type.String()),
                packageName: Type.Optional(Type.String()),
                includeVideo: Type.Optional(Type.Boolean()),
        ...
        api.registerTool({
            ...
            description: "Create a markdown launch brief from site intelligence, marketability audit, evidence map, and asset matrix.",
            parameters: Type.Object({
                intelligence: Type.Any(),
        ...
        api.registerTool({
            ...
            description: "Format image prompts and video storyboard scenes into a handoff-ready markdown document.",
            parameters: Type.Object({
                productName: Type.Optional(Type.String()),
        ...
        api.registerTool({
            ...
            description: "Create platform-specific launch copy for LinkedIn, X/Twitter, Product Hunt, Indie Hackers, email, and website banners.",
            parameters: Type.Object({
                productName: Type.String(),
                websiteUrl: Type.Optional(Type.String()),
        ...
        api.registerTool({
            ...
            description: "Create an agency/client-ready markdown handoff with scan summary, capture status, marketability, assets, approvals, and fixes.",
            parameters: Type.Object({
                productName: Type.String(),
                websiteUrl: Type.Optional(Type.String()),
        ...
        api.registerTool({
            ...
            description: "Create a polished client-facing launch readiness report from scan evidence, product audit, marketability audit, captures, assets, fixes, and optional Lovable prompts.",
            parameters: Type.Object({
