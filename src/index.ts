...
type CreativeMediaProductionPlan = {
  productName: string;
  productionMode: "prompt-only" | "render-social-images" | "render-video" | "render-full-media-pack" | "blocked";
  positioning: string;
  sourceReadiness: {
    screenshots: "missing" | "partial" | "approved";
    videoClips: "missing" | "optional" | "approved";
    claims: "needs-review" | "approved";
    screenQuality: "blocked" | "watch" | "ready";
  };
  recommendedOutputs: Array<{
    output: string;
    format: string;
    owner: "creative-studio" | "openclaw-renderer" | "image-generator" | "video-renderer" | "designer" | "user";
...
type SocialImageRenderPack = {
  productName: string;
  rendererMode: "openclaw-image-generation" | "design-tool" | "manual-designer";
  assets: Array<{
    name: string;
    format: string;
    dimensions: string;
...
type VideoRenderPack = {
  productName: string;
  rendererMode: "storyboard-only" | "screen-recording-edit" | "remotion-ffmpeg" | "video-provider";
...
type MediaAssetManifest = {
  productName: string;
  packageName: string;
  productionMode: CreativeMediaProductionPlan["productionMode"];
  files: Array<{
    path: string;
...
function makeMediaProductionPlan(params: {
  ...
  wantsRenderedImages?: boolean;
  wantsRenderedVideo?: boolean;
  approvedScreenshotRefs?: string[];
  approvedVideoClipRefs?: string[];
  supportedClaims?: string[];
  hasAssetReview?: boolean;
  hasPrivateDataRisk?: boolean;
  hasUnsupportedClaims?: boolean;
  ...
  const productionMode: CreativeMediaProductionPlan["productionMode"] =
    blockers.some((item) => !item.includes("video clips are missing"))
      ? "blocked"
  ...
  return {
    ...
    productionMode,
    positioning:
      "Create launch media from real product evidence: approved screenshots, approved clips, supported claims, and human-reviewed copy.",
    sourceReadiness: {
    ...
    recommendedOutputs: [
      ...imageFormats.map((format) => ({
        ...
        format,
        owner: productionMode === "blocked" ? "user" as const : "image-generator" as const,
        status: productionMode === "blocked" ? "blocked" as const : screenshots.length > 0 ? "ready" as const : "needs-evidence" as const,
      })),
      ...videoFormats.map((format) => ({
        output: `${params.productName} ${format}`,
        format,
        owner: clips.length > 0 ? "video-renderer" as const : "creative-studio" as const,
        status: clips.length > 0 ? "ready" as const : "needs-evidence" as const,
      })),
    ],
    renderPipeline: [
...
function dimensionsForFormat(format: string): string {
  const lower = format.toLowerCase();
  if (lower.includes("product hunt")) return "1270x760";
  if (lower.includes("linkedin")) return "1200x627";
  if (lower.includes("twitter") || lower.includes("x/")) return "1600x900";
  if (lower.includes("vertical") || lower.includes("story") || lower.includes("reel")) return "1080x1920";
  if (lower.includes("square")) return "1080x1080";
  if (lower.includes("hero")) return "1920x1080";
  return "1600x900";
}
  ...
  return {
    ...
    assets: formats.map((format, index) => {
      ...
      return {
        ...
        negativePrompt:
          "No fake app screens, no distorted screenshots, no tiny unreadable text, no private data, no unsupported claims, no fake customer logos.",
...
function makeVideoRenderPack(params: {
  ...
  const source = (index: number) => clips[index] ?? screenshots[index] ?? screenshots[0];
...
function makeMediaAssetManifest(params: {
  productName: string;
  packageName?: string;
  productionPlan?: CreativeMediaProductionPlan;
  socialImageRenderPack?: SocialImageRenderPack;
  videoRenderPack?: VideoRenderPack;
}): MediaAssetManifest {
  const packageName = params.packageName ?? "launch-media";
  const productionMode = params.productionPlan?.productionMode ?? "prompt-only";
  const files: MediaAssetManifest["files"] = [
    {
  ...
  if (params.videoRenderPack) {
    ...
    files.push({
      ...
      content: [
        ...
        "",
        ...params.videoRenderPack.timeline.map((scene) =>
          `- ${scene.time}: ${scene.scene}. Source: ${scene.source}. Motion: ${scene.motion}. Text: ${scene.onScreenText}`,
        ),
      ].join("\n"),
    });
  }
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    api.registerTool({
      ...
      description:
        "Choose whether Creative Studio should stay prompt-only, render social images, render video, or produce a full launch media pack from approved product evidence.",
      parameters: Type.Object({
        productName: Type.String(),
        websiteUrl: Type.Optional(Type.String()),
        launchGoal: Type.Optional(Type.String()),
    ...
    api.registerTool({
      ...
      description:
        "Create final social image render specs and prompts for Product Hunt, LinkedIn, X, website hero, and other campaign formats from approved screenshots.",
      parameters: Type.Object({
        productName: Type.String(),
        approvedScreenshotRefs: optionalStringArray("Approved public screenshot references."),
        supportedClaims: optionalStringArray("Evidence-backed claims approved for media."),
    ...
    api.registerTool({
      ...
      description:
        "Create final video render specs: timeline, source screenshots/clips, captions, motion notes, export targets, and review checklist for MP4 production.",
      parameters: Type.Object({
        productName: Type.String(),
        approvedScreenshotRefs: optionalStringArray("Approved public screenshot references."),
        approvedVideoClipRefs: optionalStringArray("Approved public video clip references."),
    ...
    api.registerTool({
      ...
      description:
        "Package media production outputs into handoff files for renderers: production plan, social image prompts/specs, video timeline/specs, and next actions.",
      parameters: Type.Object({
        productName: Type.String(),
        packageName: Type.Optional(Type.String()),
